...
// Caps how many data rows a single import reads - generous for a
// spreadsheet someone is hand-maintaining (the use case here), but
// keeps one upload from reading an unbounded file into memory/JSON.
const MAX_ROWS = 2000;
...
export async function POST(req: NextRequest) {
  ...
  try {
    suggestedMapping = await suggestColumnMapping(headers);
  } catch {
    // A mapping suggestion is a convenience, not a requirement - if
    // Gemini is unavailable the user just maps every field by hand.
  }
