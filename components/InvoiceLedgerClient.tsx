  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 overflow-auto border rounded">
        <table className="text-sm border-collapse min-w-[2200px]">
          ...
          <tbody>
            ...
            {invoices.length === 0 && (
              <tr>
                <td colSpan={20} className="px-3 py-10 text-center text-ink/40">
                  No invoices yet for this forwarder. Click "+ Add Invoice" to enter one.
                </td>
              </tr>
            )}
          </tbody>
...
// ---------------- Add Invoice panel: lookup by booking/container, then fill charges ----------------
...
function AddInvoicePanel({
  ...
  async function save() {
    ...
    setSaveError(null);
    try {
      const res = await fetch("/api/forwarder-invoices", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ...
          shipping_line: lookupResult?.shipping_line ?? null,
          consignee_party_id: lookupResult?.consignee_party_id ?? null,
          consignee_name: lookupResult?.consignee_name ?? null,
          pol: lookupResult?.pol ?? null,
          pod: lookupResult?.pod ?? null,
          ...form,
        }),
      });
      const json = await res.json();
    ...
    <div className="border border-accent/30 bg-accent/5 rounded p-4 mb-4">
      ...
      {lookupResult?.matched && (
        <div className="grid grid-cols-3 gap-2 text-xs mb-3 bg-white border rounded p-3">
          ...
          <div><span className="text-ink/40">Shipping Line:</span> {lookupResult.shipping_line || "—"}</div>
          <div><span className="text-ink/40">POL:</span> {lookupResult.pol || "—"}</div>
          <div><span className="text-ink/40">POD:</span> {lookupResult.pod || "—"}</div>
...
function InvoiceRow({ inv, onChange }: { inv: Invoice; onChange: (patch: Partial<Invoice>) => void }) {
  return (
    <tr className="border-b hover:bg-blue-50/30">
      ...
      <EditableCell value={String(inv.freight_charges ?? 0)} isNumber onSave={(v) => patchInvoice(inv.id, { freight_charges: Number(v) }).then(() => onChange({ freight_charges: Number(v) }))} />
      <EditableCell value={String(inv.bl_fees ?? 0)} isNumber onSave={(v) => patchInvoice(inv.id, { bl_fees: Number(v) }).then(() => onChange({ bl_fees: Number(v) }))} />
      <EditableCell value={String(inv.aes_fees ?? 0)} isNumber onSave={(v) => patchInvoice(inv.id, { aes_fees: Number(v) }).then(() => onChange({ aes_fees: Number(v) }))} />
      <EditableCell value={String(inv.extra_charges ?? 0)} isNumber onSave={(v) => patchInvoice(inv.id, { extra_charges: Number(v) }).then(() => onChange({ extra_charges: Number(v) }))} />
      <EditableCell value={String(inv.correction_charges ?? 0)} isNumber onSave={(v) => patchInvoice(inv.id, { correction_charges: Number(v) }).then(() => onChange({ correction_charges: Number(v) }))} />
      <EditableCell value={String(inv.demurrage ?? 0)} isNumber onSave={(v) => patchInvoice(inv.id, { demurrage: Number(v) }).then(() => onChange({ demurrage: Number(v) }))} />
      <Td>
        <span className="font-medium">{inv.currency} {inv.total}</span>
      </Td>
      <Td>
